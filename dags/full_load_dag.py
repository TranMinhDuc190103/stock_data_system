...
def extract_HTML_content(**kwargs):
    ...
    if latest_error_symbol in symbols:
        index = list(symbols).index(latest_error_symbol)
        symbols = symbols[index:]

    def url(symbol): return f"https://finance.yahoo.com/quote/{symbol}/profile"
    
    def filename(symbol): return f"yahoo_data_info/{symbol}.html"
    ...
    def download_html(symbol):
        try:
            ...
            # Requests
            response = session.get(url(symbol), headers=header_html)
            # Parse HTML content
            ...
            if data == []:
                ...
                minio_client.put_object(bucket_name=bucket_name_2,
                                        obj_name=filename(symbol), obj_file=f'{symbol_name}.html')
                logging.info(f"File {filename(symbol)} is empty. Put into the {bucket_name_2} bucket. Runtime: {time.time() - start}")
                ...
                minio_client.put_object(bucket_name=bucket_name,
                                        obj_name=filename(symbol), obj_file=f'{symbol_name}.html')
                logging.info(f"File {filename(symbol)} written successfully. Put into the {bucket_name} bucket. Runtime: {time.time() - start}")
            time.sleep(5)
        except Exception as e:
            logging.error(f"{symbol_name}.html could not be downloaded. Error: {e}")
            with open("dags/error_log_extract/error_log.txt", "a") as f:
                f.write(f"{symbol}\n")
            time.sleep(5)

    with ThreadPoolExecutor(max_workers=5) as executor:
        executor.map(download_html, symbols)
...
def transform_data_from_HTML(**kwargs):
    ...
    symbols = [filename.split("/")[1].split(".")[0] for filename in list_filenames]
    ...
    def transform_html(symbol):
        
        # Filename
        filename = f"yahoo_data_info/{symbol}.html"
        
        # Extract HTML content with XPATH
        html_content = minio_client.get_object(bucket_name=bucket_name_1, obj_name=filename, prefix="yahoo_data_info/").data.decode('utf-8')
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        executor.map(transform_html, symbols)
